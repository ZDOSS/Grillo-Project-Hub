...
- Automatic last-project restore for a folder-backed session reconnects through the folder-aware load path; if that folder project cannot be read, the app returns to the launcher/recent-project reconnect flow instead of quietly opening stale browser recovery as browser-local.

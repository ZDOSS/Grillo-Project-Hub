...
- **Modal-style work item detail** backed by the shared modal primitive, with full edit, checklist (with convert-to-subtask), inline comment editing, subtasks, relationship add/remove controls, app-owned permanent-delete confirmation, archive/trash/restore, and a pinned action footer
...
| `packages/ui` | 30 | AppShell, shared button and surface primitives, WorkItemModal, BoardView, BacklogView, CommandPalette, launcher, docs, settings |

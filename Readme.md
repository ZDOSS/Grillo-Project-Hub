...
- The project header now reports the actual save state: saved destination and age, unsaved changes, in-progress save, or save failure. The same header exposes **Save now** / **Retry save**, **Switch project**, and **Close project** so users do not have to infer storage state from the sidebar alone. Auto-save failures surface in-app instead of only going to the console.

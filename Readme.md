...
- The command layer now hard-fails unknown member edits, validates item references, rejects unknown mutation targets, preserves `hiddenViewIds` defaults when opening older bundles, and rejects lossy checklist reorders.
...
| Suite | Count | Notes |
| --- | --- | --- |
| `packages/core` | 36 | Domain, storage, dispatcher, export, import |

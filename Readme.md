...
- **Board** with drag-and-drop, readable lane widths, WIP limits (warn + hard modes), explicit hard-limit feedback, column-based status grouping, context-aware item creation that starts in the first board lane or active type/status filters, and whole-card link navigation that keeps visible card metadata available to assistive technology
...
- **Docs** with explicit view/edit sessions, Markdown editing, sanitized rendering, internal embeds, command-backed sections, reusable document templates, in-surface doc search, linked-work and backlink context panels, router-safe in-app navigation for preview links, stale document-route recovery, correct pane updates when switching documents, draft-preserving editor resets, dirty navigation confirmation across doc-surface routes, shared confirmation for document deletion, and safe navigation to the next active document after deleting the open one
...
- The project header now reports the actual save state: saved destination and age, unsaved changes, in-progress save, or save failure. The same header exposes **Save now** / **Retry save**, **Switch project**, and **Close project** so users do not have to infer storage state from the sidebar alone; successful manual saves add the project to launcher recents, and closing dirty or unsaved in-memory projects requires an explicit destructive confirmation. Auto-save failures surface in-app instead of only going to the console.
...
| `packages/ui` | 146 | AppShell, ProjectRouter, OverviewView, shared button/surface/modal primitives, lane layout CSS contracts, WorkItemModal attachment/reminder/custom-field/automation-preview coverage, TrashView bulk actions, BoardView, saved planning views, BacklogView, BugTriageView, MyWorkView, TableView, RoadmapView creation, CalendarView, CommandPalette, CreateItemDialog, launcher, docs explicit edit sessions, settings, automation settings |
...
| `tests/e2e` | 10 | Hybrid parity, project workflow, theme, palette, export, search, calendar scheduled-work creation, docs create/save/edit-mode behavior, mobile navigation |

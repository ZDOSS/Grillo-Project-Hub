...
- Opening a folder-backed PWA recent from the launcher reconnects the selected folder before loading the `.pms.json`; if folder access is cancelled or rejected, the browser-local recovery copy can still open, but if the selected folder is missing the recorded project file, the app asks for the folder that contains that `.pm-suite` path instead of silently opening stale recovery data.
